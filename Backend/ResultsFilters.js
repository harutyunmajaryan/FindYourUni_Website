    ...
    if (savedFilters) {
        ...
        if (filters.categories && filters.categories.length > 0) {
            filters.categories.forEach(category => {
                window.categorySelector.addTag(category);
            });
        }
        
        // Restore course names
        if (filters.courseNames && filters.courseNames.length > 0) {
            filters.courseNames.forEach(courseName => {
                window.courseSelector.addTag(courseName);
            });
        }
        ...
        if (filters.locations && filters.locations.length > 0) {
            filters.locations.forEach(location => {
                window.locationSelector.addTag(location);
            });
        }
        ...
        if (filters.universities && filters.universities.length > 0) {
            filters.universities.forEach(university => {
                window.universitySelector.addTag(university);
            });
        }
        
        // Restore degree types
        if (filters.degrees && filters.degrees.length > 0) {
            filters.degrees.forEach(degree => {
                window.degreeSelector.addTag(degree);
            });
        }
        
        // Restore grade type
        if (filters.gradeType) {
            if (filters.gradeType === 'A-Levels') {
                document.getElementById('checkbox1').checked = true;
                window.gradesSelector.fetchFunction = fetchALevelGradeOptions;
            } else if (filters.gradeType === 'IB') {
        ...
        if (filters.grades && filters.grades.length > 0) {
            filters.grades.forEach(grade => {
                window.gradesSelector.addTag(grade);
            });
        }
    }
